...
# 【中文导读】定义 Belgian 20 节点气网的压力边界、气体参数和换算常数。
@dataclass(frozen=True)
class GasConfig:
...
# 【中文导读】构建 Belgian20 气网并挂接气源、负荷、GFG、P2G、压缩机和 PRS。
...
class CoupledSolver:
    ...
    # 【中文导读】先写入电气动作，按事件决定气网求解，再逐步刷新压缩机负荷并求电网。
...
class ElectricGasMultiScaleEnv:
    ...
    # 【中文导读】截取前 94 维电侧状态作为快 Worker 基础观测。
    ...
    # 【中文导读】截取后 78 维 ESS/气侧状态作为慢 Worker 基础观测。
