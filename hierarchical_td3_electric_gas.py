...
def validate_checkpoint_compatibility(payload: Dict[str, Any], agents: AgentBundle) -> None:
    ...
    for key, expected_value in expected.items():
        ...
        if actual != expected_value:
            problems.append(f"{key} changed: expected {expected_value!r}, got {actual!r}")
    if problems:
        raise ValueError(
...
def run_training(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    for episode in range(start_episode, cfg.episodes):
        ...
        for t in range(cfg.episode_steps):
            ...
            # 环境只接收一个完整动作向量：前 11 维慢动作保持，后 16 维快动作每步更新。
...
def run_minimum_tests() -> None:
    ...
    try:
        load_checkpoint(str(old_ckpt_path), build_agents(env, cfg, device), device)
